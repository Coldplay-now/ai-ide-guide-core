...
输入提示词：
"为UserServiceImpl生成完整的单元测试代码，包括正常流程和异常情况的测试"
...
作为一个注册用户，我希望能够使用用户名和密码登录系统，以便访问个人账户信息。
    ...
    - name: Notify deployment failure
      ...
      with:
        ...
        text: |
          ...
          提交: ${{ github.sha }}
          作者: ${{ github.actor }}
          请检查构建日志: ${{ github.event.workflow_run.html_url }}
      env:
        SLACK_WEBHOOK_URL: ${{ secrets.SLACK_WEBHOOK_URL }}
...
55. 构建应用：`mvn clean package`
56. 构建镜像：`docker build -t myapp .`
57. 启动服务：`docker-compose up -d`
