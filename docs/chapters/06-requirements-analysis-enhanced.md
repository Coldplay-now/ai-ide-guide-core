...
验收标准：
7. WHEN 管理员点击"添加商品"按钮 THEN 系统 SHALL 显示商品信息录入表单
8. WHEN 管理员填写商品名称、描述、价格等必填信息 THEN 系统 SHALL 验证信息格式的正确性
9. WHEN 管理员上传商品图片 THEN 系统 SHALL 支持多张图片上传并自动压缩优化
10. WHEN 管理员保存商品信息 THEN 系统 SHALL 将商品添加到商品库并分配唯一ID
11. WHEN 商品保存成功 THEN 系统 SHALL 显示成功提示并跳转到商品列表页面

用户故事2：库存管理
作为商家管理员，我希望能够实时管理商品库存，以便避免超卖情况。

验收标准：
12. WHEN 管理员查看商品详情 THEN 系统 SHALL 显示当前库存数量和库存状态
13. WHEN 管理员修改库存数量 THEN 系统 SHALL 记录库存变更历史
14. WHEN 库存数量低于预设阈值 THEN 系统 SHALL 发送库存预警通知
15. WHEN 库存为零 THEN 系统 SHALL 自动将商品状态设置为"缺货"
16. WHEN 有新订单产生 THEN 系统 SHALL 自动扣减对应商品库存
...
输入提示词：
"检查以下CRM系统需求的一致性：
需求1：销售人员可以创建客户信息，包括公司名称、联系人、电话等基本信息
需求2：系统应该防止重复客户信息的创建
需求3：客户信息创建后，系统自动分配唯一的客户编号
需求4：销售人员可以随时修改客户的基本信息
需求5：客户信息一旦创建，不允许删除，只能标记为无效"
...
| 对比维度 | 传统需求分析方法 | AI辅助需求分析方法 | 优势分析 |
| --------- | ---------------- | ------------------- | ---------- |
| **需求收集效率** | 依赖人工访谈和文档整理，耗时较长 | AI快速生成需求模板和用户故事，效率提升60% | AI方法显著提升收集效率 |
| **需求标准化程度** | 依赖分析师经验，标准化程度不一 | 使用统一的模板和格式，标准化程度高 | AI确保需求格式一致性 |
| **需求完整性检查** | 人工检查，容易遗漏 | AI自动检查需求覆盖度和完整性 | AI减少人为遗漏风险 |
| **需求一致性验证** | 人工对比，工作量大且易出错 | AI自动识别冲突和不一致 | AI提高验证准确性 |
| **原型生成速度** | 需要专门的原型设计师，周期长 | AI快速生成可交互原型 | AI大幅缩短原型制作时间 |
...
| 检查类别 | 检查项目 | 检查标准 | 检查结果 |
| --------- | --------- | --------- | ---------- |
| **完整性** | 功能需求完整性 | 是否覆盖所有业务场景 | □通过 □不通过 |
|  | 非功能需求完整性 | 是否包含性能、安全、可用性等需求 | □通过 □不通过 |
|  | 约束条件完整性 | 是否明确技术、时间、预算约束 | □通过 □不通过 |
| **正确性** | 需求描述准确性 | 需求描述是否准确无歧义 | □通过 □不通过 |
...
通过这些模板和工具，团队可以更加规范和高效地进行需求分析工作，确保需求质量和项目成功。